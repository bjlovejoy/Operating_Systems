...
void handleCommands(char *buffer, char background, char foreground)
{
   ...
   else if(compareCommands(cmd, "copy\0"))
   {
      ...
      if(!incomplete)
      {
         /*copy file into buffer and collect file size in sectors*/
         parseWord(word, buffer, 1, 0);
         if(word[0] < 97)
            interrupt(33, 15, 1, 0, 0);
         interrupt(33, 3, word, extraBuffer, &numSectors);
         
         /*create file with new name but same contents*/
         parseWord(word, buffer, 2, 0);
         if(word[0] < 97)
            interrupt(33, 15, 1, 0, 0);
         interrupt(33, 8, word, extraBuffer, numSectors);
      }
   }
   ...
   else if(compareCommands(cmd, "exec\0"))
   {
      ...
      if(!incomplete)
      {
         interrupt(33, 4, word, 4, 0);
      }
   }
   else if(compareCommands(cmd, "help\0"))
   {
      interrupt(33, 12, background+1, foreground+1, 0);
      interrupt(33, 4, "userManual\0", 4, 0);
   }
   else if(compareCommands(cmd, "prnt\0"))
   {
      ...
      {
         interrupt(33, 3, word, extraBuffer, &numSectors);
         interrupt(33, 0, extraBuffer, 1, 0);
      }
   }
   ...
   else if(compareCommands(cmd, "show\0"))
   {
      ...
      {
         interrupt(33, 3, word, extraBuffer, &numSectors);
         interrupt(33, 0, extraBuffer, 0, 0);
         interrupt(33, 0, "\r\n\0", 0, 0);
      }
   }
   else if(compareCommands(cmd, "twet\0"))
   {
      ...
      if(!incomplete)
      {
         if(word[0] < 97)
            interrupt(33, 15, 1, 0, 0);
         interrupt(33, 0, "\n\rPlease enter text to save in new file:\0", 0, 0);
         interrupt(33, 0, "\n\r(less than 140 characters)\n\r\0", 0, 0);
         interrupt(33, 1, extraBuffer, 0, 0);
         interrupt(33, 8, word, extraBuffer, 1);
         interrupt(33, 0, "\r\n\0", 0, 0);
      }
   }
   else
